        ...
        with open(os.path.join(settings.BASE_DIR, 'companies_data.json'), 'r') as init_data:
            ...
            try:
                for company in companies:
                    ...
                    AppRequest('ZADD', settings.REDIS_LEADERBOARD, {symbol: company.get('marketCap')})
                    AppRequest('HSET', symbol, 'company', company.get('company'))
                    AppRequest('HSET', symbol, 'country', company.get('country'))
...
class CompaniesRanks:
    ...
    def update_company_market_capitalization(self, amount, symbol):
        AppRequest('ZINCRBY', settings.REDIS_LEADERBOARD, amount, self.add_prefix_to_symbol(settings.REDIS_PREFIX, symbol))
        return None
    ...
    def get_zrange(self, start_index, stop_index, desc=True):
        ...
        if desc:
            companies = AppRequest('ZREVRANGE')
        else:
            companies = AppRequest('ZRANGE')
    ...
    def get_result(self, companies, start_index=0, desc=True):
        ...
        for company in companies:
            ...
            company_info = AppRequest('HGETALL', symbol)
